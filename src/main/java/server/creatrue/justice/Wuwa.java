...
public class Wuwa extends JusticeCreature{
    private int atkCount;
    private boolean skillCount;
    public Wuwa(Map map, LinkedList<Bullet> bullets){
        super(map,bullets);
        this.name = "Wuwa";
        this.id = 4;
        this.atkCount = 0;
    ...
    public void skill(){
        if(skillCount){
            ...
            for(int i = 0; i< MapConfig.ROWS; i++){
                Bullet bullet = bulletFactory.getBullet(i* MapConfig.BLOCKSIZE+ MapConfig.BLOCKSIZE/2, MapConfig.BLOCKSIZE/2, Direction.DOWN,camp,atk,id);
                bullets.add(bullet);
                Map.skill1 = 30;
